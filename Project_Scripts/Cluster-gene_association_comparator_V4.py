#Cluster-gene association comparator_V4
#CHANGES:
...
#V4 Notes
#Updated to make output folders create themselves if not already present
#Incorporated Variation analysis code
import os
import re
...
while validchoice == False:
    ...
    if datasetprompt == "both" or datasetprompt == "Both" or datasetprompt == "BOTH":
        ...
        if os.path.isdir(outfilestem) == False:
            os.makedirs(outfilestem)
...
bsigr_to_tf_dict = {}
print("Writing collated bsIGR to gene associations from across all genomes")
    ...
    for igr in igr_assoc_data_dict.keys():
        if igr in bs_to_tf_dict.keys():
            ...
            for gene,occ_count,percent_ops,total_assocs,avglen in igr_assoc_data_dict[igr]:
                ...
                if gene in gene_to_bsigrs.keys():
                    if igr not in gene_to_bsigrs[gene]:
                        gene_to_bsigrs[gene].append(igr)
                if gene not in gene_to_bsigrs.keys():
                    gene_to_bsigrs[gene].append(igr)
                if igr not in bsigr_to_tf_dict.keys():
                    bsigr_to_tf_dict[igr] = currentTF
                if igr in bsigr_to_tf_dict.keys():
                    if currentTF not in bsigr_to_tf_dict[igr]:
                        print(f"Potential error with {igr}: Different instances bind different TFs, {bsigr_to_tf_dict[igr]} vs {currentTF}")

print("Writing Gene to bsIGR association variation analysis")
with open(str(outfilestem+"\\Gene_to_bsigr_variation_analysis.txt"),"w") as file:
            ...
            if len(gene_to_bsigrs[gene]) > 1: #If the gene is found associated to multiple different IGRs
                ...
                for igr in gene_to_bsigrs[gene]: #For each IGR the gene is found assoc to
                    if bsigr_to_tf_dict[igr] not in tmptfs: #If the TF that binds the current IGR is different to any past TFs binding previous IGRs associated to this gene
                        for clusterid,genecount,assoccount,percentassoc,assocexp in gene_dict[gene]: #Getting stats across genomes for this gene to IGR association
                            if clusterid == igr:
    ...
    file.write("###############################################################################################################################\n")
    for gene in genes_with_multiple_bsigrs.keys():
        file.write(f"\n{gene}\n")
        for igr,tf,numoccs,percentassoc,assocexp in genes_with_multiple_bsigrs[gene]:
            file.write(f"{igr}\t{tf}\t{numoccs}\t{round(percentassoc,3)}\t{round(assocexp,3)}\n")
